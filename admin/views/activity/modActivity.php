...
<nav class="navbar navbar-default child-nav">
    <h5 class="nav pull-left">修改活动</h5>
...
<form class="form-horizontal" id="modForm">
    <input type="hidden" class="form-control input-sm" name="Activity[id]" id="id" value="<?php echo $activity['id']?>">
    <div class="form-group">
        <label for="title" class="col-sm-2 control-label">活动标题<span class="mandatory">*</span></label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[title]" id="title" placeholder="活动标题" value="<?php echo $activity['title']?>">
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <ul class="upload" data-uploadid="thumb">
                <?php if(!empty($activity['thumb'])){?>
                    <li class="full"><img src="<?php echo $activity['thumb']?>"><span class="del-img"></span></li>
                <?php }else{?>
                    <li class="empty"></li>
                <?php }?>
            </ul>
            <input type="hidden" name="Activity[thumb]" id="thumb" value="<?php echo $activity['thumb']?>">
        </div>
    </div>
    <div class="form-group">
        <label for="performance_time" class="col-sm-2 control-label">演出时间</label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[performance_time]" id="performance_time" placeholder="演出时间" value="<?php echo $activity['performance_time']?>">
        </div>
    </div>
    <div class="form-group">
        <label for="copyfrom" class="col-sm-2 control-label">来源</label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[copyfrom]" id="copyfrom" placeholder="来源" value="<?php echo $activity['copyfrom']?>">
        </div>
    </div>
    <div class="form-group">
        <label for="keywords" class="col-sm-2 control-label">关键字</label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[keywords]" id="keywords" placeholder="关键字" value="<?php echo $activity['keywords']?>">
            <span class="help-block">多个关键字请用空格隔开</span>
        </div>
    </div>
    <div class="form-group">
        <label for="activity_type" class="col-sm-2 control-label">置顶</label>
        <div class="col-sm-9 iCheck">
            <label class="radio-inline">
                <input type="radio" name="Activity[activity_type]" value="0" <?php if($activity['activity_type'] === '0'){?>checked<?php }?>> 普通
            </label>
            <label class="radio-inline">
                <input type="radio" name="Activity[activity_type]" value="1" <?php if($activity['activity_type'] === '1'){?>checked<?php }?>> 置顶
            </label>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9 iCheck">
            <label class="radio-inline">
                <input type="radio" name="Activity[status]" value="0" <?php if($activity['status'] === '0'){?>checked<?php }?>> 不显示
            </label>
            <label class="radio-inline">
                <input type="radio" name="Activity[status]" value="1" <?php if($activity['status'] === '1'){?>checked<?php }?>> 显示
            </label>
        </div>
    </div>
    <div class="form-group">
        ...
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[link]" id="link" placeholder="外部链接" value="<?php echo $activity['link']?>">
        </div>
    </div>
    <div class="form-group">
        <label for="sort" class="col-sm-2 control-label">排序</label>
        <div class="col-sm-9">
            <input type="text" class="form-control input-sm" name="Activity[sort]" id="sort" placeholder="排序" value="<?php echo $activity['sort']?>">
        </div>
    </div>
    <div class="form-group">
        <label for="description" class="col-sm-2 control-label">描述</label>
        <div class="col-sm-9">
            <textarea class="form-control" name="Activity[description]" id="description" rows="2"><?php echo $activity['description']?></textarea>
        </div>
    </div>
    <div class="form-group">
        <label for="content" class="col-sm-2 control-label">内容</label>
        <div class="col-sm-9">
            <script class="editor" id="content" name="Activity[content]" type="text/plain" style="height: 250px;"><?php echo $activity['content']?></script>
        </div>
    </div>
    <div class="form-group">
...
<script type="text/javascript">
    ...
    /*修改*/
    $("#mod").click(function(){
        jajax("<?php echo Url::to(['activity/mod-activity'])?>", $('#modForm').serialize());
    })
</script>
<!-- 编辑器按钮 -->
